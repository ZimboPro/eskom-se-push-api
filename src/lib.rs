...
impl EskomAPIAsync {
    ...
    pub async fn areas_nearby_async(&self, lat: impl ToString, long: impl ToString) -> Result<AreaNearby, HttpError> {
    ...
    pub async fn topics_nearby_async(&self, lat: impl ToString, long: impl ToString) -> Result<TopicsNearby, HttpError> {
    ...
    pub async fn check_allowance_async(&self) -> Result<AllowanceCheck, HttpError> {
        let t = self.client.get(Endpoints::CheckAllowace.to_string()).send().await;
...
impl EskomAPI {
    pub fn new(token: &str) -> Self {
        ...
        headers.insert("token", reqwest::header::HeaderValue::from_str(token).unwrap());
        EskomAPI {
            client: reqwest::blocking::Client::builder().default_headers(headers).build().unwrap()
        }
    }
    ...
    /// Find areas based on GPS coordinates (latitude and longitude).
    /// The first area returned is typically the best choice for the coordinates - as it's closest to the GPS coordinates provided. However it could be that you are in the second or third area.
    pub fn areas_nearby_async(&self, lat: impl ToString, long: impl ToString) -> Result<AreaNearby, HttpError> {
        let t = self.client.get(Endpoints::AreasNearby.to_string())
            .query(&[("lat", lat.to_string()), ("lon", long.to_string())])
            .send();
    ...
    /// Find topics created by users based on GPS coordinates (latitude and longitude). Can use this to detect if there is a potential outage/problem nearby
    pub fn topics_nearby(&self, lat: impl ToString, long: impl ToString) -> Result<TopicsNearby, HttpError> {
        let t = self.client.get(Endpoints::TopicsNearby.to_string())
            .query(&[("lat", lat.to_string()), ("lon", long.to_string())])
            .send();
